...
export default function AdminAvailability() {
  ...
  const [area, setArea] = useState("prevencion");
    ...
    <div className="grid-2">
      <div className="card">
        ...
        <form className="form" onSubmit={create}>
          <label className="label">Área<select className="select" value={area} onChange={(e) => setArea(e.target.value)}><option value="prevencion">Prevención</option><option value="masoterapia">Masoterapia</option></select></label>
          ...
          <label className="label">Hora<select className="select" value={hora} onChange={(e) => setHora(e.target.value)}>{["09:00","10:00","11:00","12:00","15:00","16:00","17:00","18:00"].map((s) => <option value={s} key={s}>{s}</option>)}</select></label>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table className="table">
            <thead><tr><th>Fecha</th><th>Hora</th><th>Área</th><th>Acción</th></tr></thead>
            <tbody>
              {items.map((item) => <tr key={item.id}><td>{item.fecha}</td><td>{item.hora}</td><td>{item.area}</td><td><button className="btn red" onClick={() => remove(item.id)}>Eliminar</button></td></tr>)}
              {items.length === 0 && <tr><td colSpan={4}>No hay disponibilidad configurada.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
