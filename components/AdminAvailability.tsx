...
export default function AdminAvailability() {
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok || !data.ok) {
        throw new Error(data.error || "No se pudo crear disponibilidad.");
      }
      setMessage("Disponibilidad guardada.");
      await load();
    } catch (err: any) {
      setError(err.message || "No se pudo crear disponibilidad.");
    }
  }

  async function remove(id: string) {
    try {
      const response = await fetch(`/api/admin/disponibilidad?id=${id}`, {
        method: "DELETE"
      });
      const data = await response.json();
      if (!response.ok || !data.ok) {
        ...
        <form className="form" onSubmit={create}>
          ...
          <label className="label">
            ...
            <select className="select" value={hora} onChange={(e) => setHora(e.target.value)}>
              {["09:00", "10:00", "11:00", "12:00", "15:00", "16:00", "17:00", "18:00"].map((slot) => (
        ...
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={4}>No hay disponibilidad configurada.</td>
                </tr>
              )}
            </tbody>
          </table>
