...
export default function BookingForm({ type, title, services }: BookingFormProps) {
  ...
  useEffect(() => {
    if (!date && days.length > 0) {
      setDate(toISODate(days[1] || days[0]));
    }
  }, [date, days]);
  ...
  return (
    <div className="calendar-layout">
      <div className="card calendar-panel">
        <div>
          <h2>{title}</h2>
          <p>Elige el servicio, selecciona un día disponible y luego una hora.</p>
        </div>
        ...
        <div>
          ...
          <div className="day-strip" style={{ marginTop: 12 }}>
            {days.map((day) => {
              ...
              return (
                <button className={active ? "day-card active" : "day-card"} type="button" key={iso} onClick={() => setDate(iso)}>
