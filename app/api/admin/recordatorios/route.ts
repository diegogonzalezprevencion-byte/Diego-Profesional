...
export async function GET(request: Request) {
  ...
  if (type === "24h") {
    query = query.eq("fecha", tomorrow).eq("recordatorio_24h_enviado", false);
  }

  if (type === "same_day") {
    query = query.eq("fecha", today).eq("recordatorio_dia_enviado", false);
  }
  ...
  const reminders = (data || []).map((reserva) => {
    const reminderType = reserva.fecha === tomorrow ? "24h" : reserva.fecha === today ? "same_day" : "future";
    const message = reminderType === "same_day"
      ? reminderSameDayMessage(reserva)
      : reminder24hMessage(reserva);

    return { ...reserva, reminderType, reminderMessage: message };
  });

  return NextResponse.json({ ok: true, reminders, today, tomorrow });
}
