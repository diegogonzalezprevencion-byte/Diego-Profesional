...
export async function GET() {
  ...
  const { data, error } = await client.from("reservas").select("*").order("fecha", { ascending: true }).order("hora", { ascending: true });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reserva: data,
      automation: {
        whatsappMessage: `Hola, soy ${body.nombre}. Solicité una reserva para ${body.area}: ${body.servicio}, el día ${body.fecha} a las ${body.hora}. Quisiera confirmar disponibilidad.`,
        emailPrepared: true
      }
    }, { status: 201 });
  } catch {
